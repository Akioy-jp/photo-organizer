...
class PhotoProcessor:
    ...
    def load_config(self, config_path: str) -> dict:
        # exe化された場合も、スクリプト実行の場合も、実行ファイルと同じ場所のconfig.jsonを探す
        ...
        try:
            ...
            print(f"Error: Configuration file not found: {resolved}")
            print("config.json を PhotoOrganizer.exe と同じフォルダに置いてください。")
            input("Enterキーを押すと終了します...")
            sys.exit(1)
        except json.JSONDecodeError as e:
            print(f"Error: config.json の形式が正しくありません: {e}")
            input("Enterキーを押すと終了します...")
            sys.exit(1)
    ...
    def detect_qr_code(self, image_path: Path) -> Optional[str]:
        """OpenCV WeChatQRCode（高精度）+ QRCodeDetector（フォールバック）でQRを検出する。
        pyzbar/zbar は使用しないため、DLLエラーは発生しない。
        """
        try:
            ...
            # --- 第1試行: WeChatQRCode（遠距離・斜め撮影に強い高精度エンジン）---
            try:
                ...
                pass  # WeChatQRCodeが利用できない環境ではスキップ

            # --- 第2試行: 標準QRCodeDetector（グレースケール）---
